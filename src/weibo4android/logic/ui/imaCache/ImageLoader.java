...
public class ImageLoader {
    // 一个map，用于缓存下载过的图片
    private HashMap<String, SoftReference<Bitmap>> mCaches;
    // 任务队列
        ...
        public void run() {
            ...
            while (true) {
                ...
                while (mTaskQueue.size() > 0) {
                    ...
                    // Log.i("msg", "in thread: " + taskQueue.size() + ":" +
                    // task.path);
                    try {
                        // 下载图片
                        // InputStream in = HttpTool.getStream(task.path, null,
                        // null, HttpTool.GET);
                        // task.bitmap = BitmapTool.getBitmap(in, 2);
                        // 将下载的图片添加到缓存
                        task.bitmap = PicUtill.getbitmap(task.path);
                        mCaches.put(task.path, new SoftReference<Bitmap>(task.bitmap));
    ...
    public ImageLoader() {
        ...
        mTaskQueue = new ArrayList<ImageLoader.Task>();
        // 启动下载任务线程
        thread.start();
    }
    ...
    public Bitmap loadImage(final String path, int scale, final ImageCallback callback) {
        ...
        if (mCaches.containsKey(path)) {
            // 取出软引用
            SoftReference<Bitmap> rf = mCaches.get(path);
            // 通过软引用，获取图片
        ...
        if (!mCaches.containsKey(path)) {
            ...
            if (!mTaskQueue.contains(task)) {
                ...
                // Log.i("msg", "in loadImage : " + taskQueue.size() +
                // task.path);
            }
